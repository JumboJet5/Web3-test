...
export const getIndexes = async (groups) => {
  try {
    return Promise.all(groups.map(async (group) => {
      const _indexes = await Promise.all(group.indexes.map(index => contract.methods.getIndex(index).call()))
      group.indexes = _indexes
      return group
    }))
  } catch (e) {
    console.error(e);
...
export const getDataFromIndexes = async () => {
  try {
    ...
    return await getIndexes(groups);
  } catch (e) {
    console.error(e);
  }
}
